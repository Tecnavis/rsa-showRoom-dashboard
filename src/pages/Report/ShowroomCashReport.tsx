...
const ShowroomCashReport: React.FC = () => {
      ...
      try {
        ...
        const q = query(
          collection(db, `user/${uid}/bookings`),
                        where('serviceCategory', '==', 'showroom'),
...
setBookings(sortedBookingsData);
  ...
  return (
    ...
    <h2 className="text-center uppercase text-3xl font-bold text-gray-900 p-6 shadow-lg rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 border border-gray-300 tracking-wider">
      Cash Report
    </h2>
...
<div className="grid grid-cols-4 md:grid-cols-4 gap-6 my-6">
  {[
    {
    ...
    {
      label: selectedMonth !== null ? `Monthly Total Balance (${months[selectedMonth]}):` : "Monthly Total Balance:",
      value: balanceMonthlyTotal.toFixed(2),
    },
    { label: "Whole Total:", value: totalAmount.toFixed(2) },
    { label: "Whole Total Balance:", value: balanceTotal.toFixed(2) },
  ].map((item, index) => (
    <div
      key={index}
      className={`shadow-lg rounded-xl p-6 border transform transition-all duration-300 hover:scale-105 ${cardColors[index]}`}
    >
...
export default ShowroomCashReport;